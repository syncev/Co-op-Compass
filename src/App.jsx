...
function App() {
  ...
  function confirmFilter() {
    const genresArray = Array.from(genresRef.current).filter(
      (input) => input.checked
    );
    var genresIdArray = "";
    const genresNameArray = genresArray.map((genre) => genre.name).join(", ");
    if (genresArray.length > 0) {
      genresIdArray = `&genres=${genresArray
        .map((genre) => genre.id)
        .join(",")
        .toLowerCase()}`;
    }

    const platformsArray = Array.from(platformsRef.current).filter(
      (input) => input.checked
    );
    var platformsIdArray = "";
    const platformsNameArray = platformsArray
      .map((platform) => platform.name)
      .join(", ");
    if (platformsArray.length > 0) {
      platformsIdArray = `&platforms=${platformsArray
          ...
          <div className="filterPicker hidden" ref={filterPicker}>
            ...
            <div className="filterPicker-categories">
              <div className="filterCategories-wrapper">
                <h3>Categories</h3>
                <form action="" ref={genresRef} id="genresForm"></form>
              </div>
              <div className="filterConsoles-wrapper">
                <h3>Platforms</h3>
                <form action="" ref={platformsRef} id="platformsForm"></form>
              </div>
            </div>
          </div>
        </div>
